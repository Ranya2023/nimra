...
(() => {
  ...
  const S = {
    lang: storage.get(LS.lang, 'ku') === 'en' ? 'en' : 'ku',
    ...
    activityId: null,
    dirty: new Map(),
    report: { onlyReleased: false, includeActivities: true, sort: 'roster' },
    installEvent: null,
    seq: 0,
  ...
  function dateText(ms, withTime) {
    ...
    try {
      return new Intl.DateTimeFormat(S.lang === 'ku' ? 'ckb-IQ' : 'en-GB', Object.assign({ numberingSystem: 'latn' }, opts)).format(new Date(ms));
    } catch (e) {
  ...
  function applyLang() {
    document.documentElement.lang = S.lang === 'ku' ? 'ckb' : 'en';
    document.documentElement.dir = S.lang === 'ku' ? 'rtl' : 'ltr';
    document.title = S.lang === 'ku' ? 'نمرە' : 'Nimre';
  }

  function setLang(lang) {
    S.lang = lang === 'en' ? 'en' : 'ku';
  ...
  async function api(action, payload = {}) {
    if (!CFG.API_URL || CFG.API_URL.indexOf('PASTE_') === 0) throw new ApiError('NOT_CONFIGURED');
    let res;
    try {
      res = await fetch(CFG.API_URL, {
        method: 'POST',
        headers: { 'Content-Type': 'text/plain;charset=utf-8' },
        body: JSON.stringify(Object.assign({ action, token: S.token, lang: S.lang }, payload)),
        redirect: 'follow'
      });
    } catch (e) {
    ...
    let data;
    try { data = await res.json(); } catch (e) { throw new ApiError('SERVER_ERROR'); }
    if (!data || !data.ok) {
      const err = new ApiError((data && data.error) || 'SERVER_ERROR', data && data.data);
      if (err.code === 'AUTH') setToken('');
      throw err;
    }
    return data;
  }
  ...
  function langButton() {
    return '<button type="button" class="lang-btn" data-act="lang" lang="' + (S.lang === 'ku' ? 'en' : 'ckb') + '">' + esc(t('langSwitch')) + '</button>';
  }
  ...
  window.Nimre = {
    CFG, LS, TYPES, TYPE_COLORS, SUBJECT_COLORS, LEVELS, S, storage,
    t, esc, r2, fmt, normDigits, metaJoin, levelOf, dateText, applyLang, setLang, appBase, isIOS, isStandalone,
    ApiError, api, errText, setToken,
    ICON, logo, ringPath,
    setView, ACTIONS, FORMS, CHANGES, INPUTS,
    toast, openSheet, closeSheetOf, closeAllSheets, confirmBox, formError, busy, copyText, loadingBlock, langButton,
    guardDirty, go
  };
})();
